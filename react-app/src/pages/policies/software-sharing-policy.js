...
<p>This policy is based on the ENCODE software release policy (https://www.encodeproject.org/about/data- use-policy/).</p>

<p><i>Rationale: To ensure reproducibility of analyses and to encourage reuse of software.</i></p>
...
<p>Software tools for data analysis and processing should be released as soon as they are sufficiently stable and <u>no later than the time of the data release</u> (according to the Tier system described at the beginning of this document).</p>
...
<p><b>Software that has been released only within the consortium (Tier 1) cannot be used in any publications of consortium members, whether for projects funded by Morphic Consortium or otherwise.</b> By requiring that any software used to generate hypotheses or results presented in a publication is publicly available (Tier 2), we will enable the broader community to repeat studies and reproduce findings.</p>

<h2>How to Release</h2>

<p>Tier 2 (i.e. released to the public) software should be released by <u>version controlled public repositories</u> (e.g. Github, GitLab, BitBucket). These repositories should be linked via the Morphic Consortium DRACC. Software should be well-documented and there should <u>be a contact person for questions</u>. Software development should continue through version-controlled deposition, and the software version used to generate each dataset must be documented using a consistent versioning scheme, e.g. Semantic Versioning (http://www.semver.org).</p>

<p>Whenever possible, MorPhiC Consortium funded software should be released under an open source license. It is, however, the sole responsibility of consortium members to follow any relevant rules of their institution with respect to software licensing.</p>

<p>In order to avoid multiple versions of the same software being maintained by different consortium members, consortium members who wish to contribute modifications of shared software tools (both Tier 1 and Tier 2) should do so using standard collaborative software development procedures such as Pull Requests on GitHub repositories. They should also clearly document what changes they made and the rationale for the modifications.</p>

<h2>Who Releases</h2>

<p>Developers who release software publicly (Tier 2) should ensure that their software is included in the MorPhiC Consortium resource catalog.</p>

<h2>Other Considerations</h2>

<p><b>Accompanying publications:</b> In addition to the release of well-documented code, we strongly encourage developers to publish citable descriptions of their software. We recommend that authors describe their software in methodological papers so that they can receive credit for their work. These can be published in conventional journals, and/or disseminated pre-publication through pre-print servers (e.g. bioRxiv).</p>

<p><b>Dissemination of more complex pipelines:</b> For most complex analyses, multiple software components are routinely combined to generate intermediate datasets. For reproducibility of these results, analysts should document all software components used, and the specific software versions utilized. We encourage (1) documenting these components; (2) providing scripts that reproduce key figures in MorPhiC Consortium publications; (3) establishing reusable, publicly accessible analysis pipelines (e.g. Biodepot, virtual machines, Docker, workflows compatible with systems provided by (commercial) cloud-based analysis providers); and (4) linking these through the MorPhiC website.</p>

<p><b>Current and future support for released software:</b> Tier 2 Software that will be released for publication and to repositories (e.g., Github) should state the types and degree of support users can expect for them to download, run, and troubleshoot the available software as well as whether or not updates and “fixes” should be expected. In many cases, the expectation is that there is minimal support for users, ie, the </p>

<p>software is provided for reproducibility purposes, and not for outside use. The state of the software should be documented in a README file that is part of the software.</p>
